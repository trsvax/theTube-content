...
- Reads body → stores to S3 at `/shares/{date}-{hash}.{ext}`
- Returns `{"src": "/shares/2026-05-24-abc123.jpg"}`
...
`/shares/{date}-{hash}.{ext}` is the current plan. But should it be under a role path? `/public/shares/...` vs `/user/shares/...`? If a share is in a protected post, should the image also be protected?
